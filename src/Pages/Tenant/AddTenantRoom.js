...
import { getSingleRoom } from "../../actions/roomActions";
...
const AddTenantRoom = () => {
  ...
  useEffect(() => {
    if (forceUpdate) {
      (async () => {
        let data = await getSingleRoom(user.userId,user.propertyId,roomId)
        setTenant({
          ...tenant,
          room: data.name,
        });

        setRent(data ? data.rate : 0);
        setTenantRentDue({
          ...tenantRentDue,
          rent: data ? data.rate : 0,
          total: calculateDue(data ? data.rate : 0, day.date, day.maxDays),
          due: calculateDue(data ? data.rate : 0, day.date, day.maxDays),
        });
    ...
    return (
      <div className="tenantMain">
        <Header name="Add Tenant" link="/room" type="back" />
        <div className="tenantSection">
          ...
          <div className="tenantInput">
            <p>Tenant Room</p>
            <input type="text" name="room" value={tenant.room} readOnly/>
          </div>
          <div className="tenantInput">
            <p>Date of Booking</p>
